...
const TABLE_HEAD = [
  { id: 'invoiceNumber', label: 'Mangas' },
  { id: 'createDate', label: 'Create' },
  { id: 'dueDate', label: 'Update' },
  { id: 'price', label: 'Amount' },
  { id: 'sent', label: 'Episodes', align: 'center' },
...
export default function InvoiceListView() {
  ...
  useEffect(() => {
    const getMangas = async () => {
      const apiUrl = 'https://starfish-app-z9xun.ondigitalocean.app/mangas';

      const res = await fetch(apiUrl, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'api-key': 'ALeniEtrITIlSiDEsTORTine',
        },
      });
      console.log(res);
      if (res.ok) {
      ...
      // const mangas = data.list.map((manga: any) => ({
      //   _id: manga._id,
      //   title: manga.title,
                ...
                <TableBody>
                  ...
                  {mangas.map((manga) => (
                    <InvoiceTableRow
                      key={manga._id}
                      row={manga}
                      selected={table.selected.includes(manga._id)}
                      onSelectRow={() => table.onSelectRow(manga._id)}
                      onViewRow={() => handleViewRow(manga._id)}
                      onEditRow={() => handleEditRow(manga._id)}
                      onDeleteRow={() => handleDeleteRow(manga._id)}
                    />
                  ))}
